...
export default (isConsole: boolean) => function formatter(_: LogRecord): string {
  ...
  let
    ...
    path: string[] | undefined = 0 in _.args ? [_.args[0] as string] : undefined;
  let r: string =
    '    ' + forConsole(colors.white)(Array.prototype.concat.apply([time, lvel, name], path as any[]).map(wrap).join(' ')) + '\n'+
    _.msg +
    '\n';
  return r;
}
